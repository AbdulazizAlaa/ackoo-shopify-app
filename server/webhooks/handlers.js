...
class OrderPaidHandler extends Handler {
  async handle(ctx, data, shop) {
    ...
    console.log(
      `Order Paid: create time (${order.createdAt}) - update time (${order.updatedAt}) - close time (${order.closedAt})`
    );
    console.log(
      "--------------------------------------------------------------"
    );
    ...
    const sessionToken = await getOrderUTMSource(ctx).catch((e) => {
      console.log(e);
      throw e;
    });
    ...
    if (sessionToken !== undefined && sessionToken !== null) {
      ...
      const res = await axios
        .put(
          `https://staging.ackoo.app/partner/transactions`,
          ReqBody,
          ReqConfig
        )
        .catch((e) => {
